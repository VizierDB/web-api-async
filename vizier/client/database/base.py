# Copyright (C) 2018 New York University,
#                    University at Buffalo,
#                    Illinois Institute of Technology.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

"""The Vizier database client enables access to and manipulation of datasets in
a vizier datastore. The main use for this class is within Python scripts that
are executed in Python cells of a notebook.

Depending on the level of concern regarding security there are different
implementations for the database client. The default implementation uses an
instance of the vizier.datastore.base.Datastore class directly to access
datasets. This causes a security risk as it potentially allows users to access
the file system of the server from within Python cells of a notebook. This
solution is only recommended in a safe environment, i.e., where the user is
running the vizier api and front end on their own computer and they are the
only user.

Once access is granted to external users an indirect client is preferred that
comminucates with the sever API via sockets or HTTP.
"""

from vizier.core.util import is_valid_name
from vizier.datastore.base import max_column_id, max_row_id
from vizier.client.database.dataset import VizierDataset
from vizier.datastore.metadata import DatasetMetadata


class DatabaseClient(object):
    """The database client provides access to datasets that are defined in the
    the current database state. It also allows to modify the database state by
    creating, updating, renaming, and deleting datasets.

    From a users point of view, each dataset in the database state is identified
    by a unique name.
    """
    @abstractmethod
    def create_dataset(self, name, dataset):
        """Create a new dataset with given name.

        Raises ValueError if a dataset with given name already exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        dataset : vizier.client.datastore.dataset.VizierDataset
            Dataset object

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        raise NotImplementedError

    @abstractmethod
    def drop_dataset(self, name):
        """Remove the dataset with the given name.

        Raises ValueError if no dataset with given name exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        """
        raise NotImplementedError

    @abstractmethod
    def get_dataset(self, name):
        """Get handle to interact with a dataset in the current database state.
        The requested dataset is identified by the given name.

        Raises ValueError if no dataset with the given name exists in the
        current database state.

        Parameters
        ----------
        name : string
            Unique dataset name

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        raise NotImplementedError

    @abstractmethod
    def new_dataset(self):
        """Get a handle for a new dataset. The dataset is not created in the
        database state until create_dataset is called with the returned handle
        as argument. The result is merley an in-memory copy for a new dataset.

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        raise NotImplementedError

    @abstractmethod
    def rename_dataset(self, name, new_name):
        """Rename an existing dataset.

        Raises ValueError if no dataset with the given name exists in the
        current database state. Raises ValueError if a dataset with new name
        already exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        new_name : string
            New dataset name
        """
        raise NotImplementedError

    @abstractmethod
    def update_dataset(self, name, dataset):
        """Update a given dataset.

        Raises ValueError if the specified dataset does not exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        dataset : vizier.datastore.base.Dataset
            Dataset object

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        raise NotImplementedError


class VizierDBClient(DatabaseClient):
    """The default database client uses a common datastore instance to access
    and manipulate datasets.
    """
    def __init__(self, datastore, datasets):
        """Initialize the reference to the datastore and the mapping of
        user-defined dataset names to unique internal identifier.

        Parameters
        ----------
        datastore: vizier.datastore.base.Datastore
            Data store to access and manipulate datasets
        datasets: dict
            Mapping of dataset names to unique persistent dataset identifier
            generated by the data store.
        """
        self.datastore = datastore
        self.datasets = datasets

    def create_dataset(self, name, dataset):
        """Create a new dataset with given name.

        Raises ValueError if a dataset with given name already exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        dataset : vizier.client.datastore.dataset.VizierDataset
            Dataset object

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        # Raise an exception if a dataset with the given name already exists or
        # if the name is not valid
        if self.has_dataset_identifier(name):
            raise ValueError('dataset \'' + name + '\' already exists')
        if not is_valid_name(name):
            raise ValueError('invalid dataset name \'' + name + '\'')
        columns = dataset.columns
        rows = dataset.rows
        column_counter = max(max_column_id(columns) + 1, 0)
        row_counter = max(max_row_id(rows) + 1, 0)
        # Ensure that all columns have a positive identifier
        for col in columns:
            if col.identifier < 0:
                col.identifier = column_counter
                column_counter += 1
        # Ensure that all rows have a positive identifier
        for row in rows:
            if row.identifier < 0:
                row.identifier = row_counter
                row_counter += 1
        # Write dataset to datastore and add new dataset to context
        ds = self.datastore.create_dataset(
            columns=columns,
            rows=rows,
            column_counter=column_counter,
            row_counter=row_counter,
            annotations=dataset.annotations
        )
        self.set_dataset_identifier(name, ds.identifier)
        return VizierDataset(dataset=ds)

    def drop_dataset(self, name):
        """Remove the dataset with the given name.

        Raises ValueError if no dataset with given name exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        """
        # Remove the context dataset identifier for the given name. Will raise
        # a ValueError if dataset does not exist
        self.remove_dataset_identifier(name)

    def get_dataset(self, name):
        """Get handle to interact with a dataset in the current database state.
        The requested dataset is identified by the given name.

        Raises ValueError if no dataset with the given name exists in the
        current database state.

        Parameters
        ----------
        name : string
            Unique dataset name

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        # Get identifier for the dataset with the given name. Will raise an
        # exception if the name is unknown
        identifier = self.get_dataset_identifier(name)
        # Read dataset from datastore and return it.
        dataset = self.datastore.get_dataset(identifier)
        if dataset is None:
            raise ValueError('unknown dataset \'' + identifier + '\'')
        return VizierDataset(dataset=dataset)

    def get_dataset_identifier(self, name):
        """Returns the unique identifier for the dataset with the given name.

        Raises ValueError if no dataset with the given name exists.

        Parameters
        ----------
        name: string
            Dataset name

        Returns
        -------
        string
        """
        # Datset names should be case insensitive
        key = name.lower()
        if not key in self.datasets:
            raise ValueError('unknown dataset \'' + name + '\'')
        return self.datasets[key]

    def has_dataset_identifier(self, name):
        """Test whether a mapping for the dataset with the given name exists.

        Parameters
        ----------
        name: string
            Dataset name

        Returns
        -------
        bool
        """
        # Dataset names are case insensitive
        return name.lower() in self.datasets

    def new_dataset(self):
        """Get a handle for a new dataset. The dataset is not created in the
        database state until create_dataset is called with the returned handle
        as argument. The result is merley an in-memory copy for a new dataset.

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        return VizierDataset()

    def remove_dataset_identifier(self, name):
        """Remove the entry in the dataset distionary that is associated with
        the given name. Raises ValueError if not dataset with name exists.

        Parameters
        ----------
        name: string
            Dataset name
        identifier: string
            Unique identifier for persistent dataset
        """
        # Convert name to lower case to ensure that names are case insensitive
        key = name.lower()
        if not key in self.datasets:
            raise ValueError('unknown dataset \'' + name + '\'')
        del self.datasets[key]

    def rename_dataset(self, name, new_name):
        """Rename an existing dataset.

        Raises ValueError if no dataset with the given name exists in the
        current database state. Raises ValueError if a dataset with new name
        already exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        new_name : string
            New dataset name
        """
        # Raise exception if new_name exists or is not valid.
        if self.has_dataset_identifier(new_name):
            raise ValueError('dataset \'' + new_name + '\' exists')
        if not is_valid_name(new_name):
            raise ValueError('invalid dataset name \'' + new_name + '\'')
        # Raise an exception if no dataset with the given name exists
        identifier = self.get_dataset_identifier(name)
        self.remove_dataset_identifier(name)
        self.set_dataset_identifier(new_name, identifier)

    def set_dataset_identifier(self, name, identifier):
        """Sets the identifier to which the given dataset name points.

        Parameters
        ----------
        name: string
            Dataset name
        identifier: string
            Unique identifier for persistent dataset
        """
        # Convert name to lower case to ensure that names are case insensitive
        self.datasets[name.lower()] = identifier

    def update_dataset(self, name, dataset):
        """Update a given dataset.

        Raises ValueError if the specified dataset does not exist.

        Parameters
        ----------
        name : string
            Unique dataset name
        dataset : vizier.datastore.base.Dataset
            Dataset object

        Returns
        -------
        vizier.client.datastore.dataset.VizierDataset
        """
        # Get identifier for the dataset with the given name. Will raise an
        # exception if the name is unknown
        identifier = self.get_dataset_identifier(name)
        # Read dataset from datastore to get the column and row counter.
        source_dataset = self.datastore.get_dataset(identifier)
        if source_dataset is None:
            raise ValueError('unknown dataset \'' + identifier + '\'')
        column_counter = source_dataset.column_counter
        row_counter = source_dataset.row_counter
        # Update column and row identifier
        columns = dataset.columns
        rows = dataset.rows
        # Ensure that all columns has positive identifier
        for col in columns:
            if col.identifier < 0:
                col.identifier = column_counter
                column_counter += 1
        # Ensure that all rows have positive identifier
        for row in rows:
            if row.identifier < 0:
                row.identifier = row_counter
                row_counter += 1
        # Write dataset to datastore and add new dataset to context
        ds = self.datastore.create_dataset(
            columns=columns,
            rows=rows,
            column_counter=column_counter,
            row_counter=row_counter,
            annotations=dataset.annotations
        )
        self.set_dataset_identifier(name, ds.identifier)
        return VizierDataset(dataset=ds)
