...
"""Implementation of the task processor for the R package."""
...
import vizier.engine.packages.r.base as cmd
import vizier.mimir as mimir


class RTaskProcessor(TaskProcessor):
    """Implementation of the task processor for the r package."""
    def compute(self, command_id, arguments, context):
        """Execute the R script that is contained in the given arguments.
        ...
        """
        if command_id == cmd.R_CODE:
            return self.execute_script(
                args=arguments,
                context=context
            )
        else:
            raise ValueError('unknown r command \'' + str(command_id) + '\'')
