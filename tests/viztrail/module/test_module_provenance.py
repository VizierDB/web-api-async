...
class FakeDatastore(object):
    def get_descriptor(self, identifier):
        if identifier in ['666', '999']:
            return DatasetDescriptor(identifier=identifier)
        else:
...
class TestModuleProvenance(unittest.TestCase):
    ...
    def adjust_state(self):
        ...
        state = prov.adjust_state(datasets=datasets, datastore=FakeDatastore())
    ...
    def test_requires_exec(self):
        ...
        self.assertTrue(ModuleProvenance(write={'A':'789'}, delete=['A']).requires_exec(datasets))
        # If the module modifies a dataset that it doesn't read but that does
        # exist the result is True
        prov = ModuleProvenance(read={'A':'123'}, write={'C':'567'}, delete=['A'])
        ...
        # state
        prov = ModuleProvenance(read={'A':'123'}, write={'A':'abc'}, delete=['A'])
        self.assertFalse(prov.requires_exec(datasets))
        ...
        prov = ModuleProvenance(read={'B':'345', 'C':'567'}, write={'B': '345'})
        self.assertFalse(prov.requires_exec(datasets))
        prov = ModuleProvenance(read={'B':'345', 'C':'567'}, write={'B': '345'}, delete=['D'])
        self.assertTrue(prov.requires_exec(datasets))
